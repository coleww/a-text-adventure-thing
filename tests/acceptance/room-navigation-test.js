...
test('room is inaccessible without its key', function(assert){
  ...
  var locked_room = server.create('room', {description: "awesome", locked: true, north: 2});
...
test('locked room messages go away', function(assert){
  assert.expect(3);
  var other_room = server.create('room', {south: 3});
  var locked_room = server.create('room', {locked: true, north: 3});
